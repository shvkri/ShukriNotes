...
const SESSION_DURATION_SECONDS = 30 * 86400; // Session validity: 30 days
...
/**
 * Unified API request handler and router
 */
async function handleApiRequest(request, env) {
	const { pathname } = new URL(request.url);

	// --- Public share routes ---
	// Match share page /share/some-uuid
	const sharePageMatch = pathname.match(/^\/share\/([a-zA-Z0-9-]+)$/);
	if (sharePageMatch) {
		const publicId = sharePageMatch[1];
		// Build target URL, set publicId as query parameter
	...
	// Match public API for getting share content /api/public/note/some-uuid
	...
	// Match public API for getting raw share content /api/public/note/raw/some-uuid
	...
	// Public file access route (must be before authentication)
	...
	// --- From here on, all APIs require authentication ---
	...
	if (pathname.startsWith('/api/docs')) {
		...
		// Match all /api/docs/node/:id related requests
...
async function handleStatsRequest(request, env) {
	...
	try {
		...
		// Use Promise.all to execute all queries in parallel for best performance
...
/**
 * Handle timeline data request, return structured note count stats by Year -> Month -> Day
 */
async function handleTimelineRequest(request, env) {
	...
	try {
		...
		// D1 doesn't directly support strftime or to_char, we need to get all timestamps and process in JS
		// Note: Performance might drop for huge datasets (hundreds of thousands of notes).
		// Acceptable for thousands to tens of thousands of notes.
		const stmt = db.prepare("SELECT updated_at FROM notes ORDER BY updated_at DESC");
		const { results } = await stmt.all();
		if (!results) {
			return jsonResponse({});
		}
		const timezoneFormatter = new Intl.DateTimeFormat('en-US', { // 'en-US' is just for formatting
			timeZone: timezone,
			year: 'numeric',
			month: 'numeric',
			day: 'numeric',
		});
		// Grouping and tallying in JS
		const timeline = {};
...
/**
 * Handle full-text search request, supports pagination and multiple filter conditions
 */
async function handleSearchRequest(request, env) {
	const { searchParams } = new URL(request.url);
	const query = searchParams.get('q');

	// 1. Fallback to handleNotesList if query is empty or only whitespace
	if (!query || query.trim().length === 0) {
		return handleNotesList(request, env);
	}
	// 2. Keep check for too short queries
...
async function handleTagsList(request, env) {
	...
	try {
		...
		const stmt = db.prepare(`
            ...
            HAVING count > 0 -- Only return used tags
...
/**
 * Handle login request (Password only)
 */
...
/**
 * Get user settings from KV. Return defaults if not in KV.
 */
async function handleGetSettings(request, env) {
	const defaultSettings = {
		...
		showHeatmap: true, // Show heatmap by default
		imageUploadDestination: 'local', // Use R2 by default
		imgurClientId: '',
		surfaceColor: '#ffffff',
		surfaceColorDark: '#1a1a1a',
		surfaceOpacity: 1,
		backgroundOpacity: 1, // Fully opaque by default
		backgroundImage: '', // Removed bg.jpg to support solid colors
		backgroundBlur: 0,
		waterfallCardWidth: 320,
		enableDateGrouping: false,
		telegramProxy: false,
		showFavorites: true,  // Control favorites
		showArchive: true,    // Control archive
		enablePinning: true,  // Control pinning
		enableSharing: true,  // Control sharing
		showDocs: true,       // Control docs link
...
/**
 * Handle GET and POST for notes list
 */
	...
	try {
		switch (request.method) {
            ...
            `;
				...
				// Add pagination params to bindings
				bindings.push(limit + 1, offset);
			...
			case 'POST': {
				...
				// Update files field if non-image attachments exist
				...
				// Return complete note to frontend
...
/**
 * Handle PUT and DELETE for single note
 */
		...
		switch (request.method) {
			case 'PUT': {
				...
				if (formData.has('content')) {
					...
					// --- File processing only cares about non-images ---
					// Process attachment deletion
					...
					if (content.trim() === '' && currentFiles.length === 0 && !hasNewFiles) {
						// Note is empty, delete it
						// 1. Delete remaining R2 files
				...
				if (formData.has('isPinned')) { // --- Pin status update ---
...
async function handleFileRequest(noteId, fileId, request, env) {
	...
	// Don't return 404 immediately even if files is empty, because image might be in pics field
	let files = [];
	...
	// Try getting file from R2
	const object = await env.NOTES_R2_BUCKET.get(`${id}/${fileId}`);
	if (object === null) {
		// Return 404 if file is not in R2
		return new Response('File not found in storage', { status: 404 });
	}

	const headers = new Headers();
	object.writeHttpMetadata(headers); // Write metadata from R2 object
	headers.set('etag', object.httpEtag);
	headers.set('Cache-Control', 'public, max-age=86400, immutable');

	// --- Set headers based on fileMeta ---
	if (fileMeta) {
		...
		// Case 2: Metadata missing, Telegram image
		// Content-Type set via R2 metadata. Usually enough for browser rendering.
		// Set inline to display in img tag
...
/**
 * Convert Telegram formatted entities to Markdown
 */
function telegramEntitiesToMarkdown(text, entities = []) {
	...
	// Priority determines tag nesting order. Smaller number means outer layer.
	const tagPriority = {
		'text_link': 10,
		'bold': 20,
		'italic': 30, // Use _ for italic to avoid conflict with **
		'underline': 40,
		'strikethrough': 50,
		'spoiler': 60,
		'code': 70,
	...
	for (let i = 0; i <= text.length; i++) {
		...
		// - Close tags from high to low priority (inner first)
		// - Open tags from low to high priority (outer first)
		...
		closeTags.forEach(({ tag }) => {
			if (adjacentSensitiveTags.includes(tag) && result.endsWith(tag)) {
				result += '\u200B'; // Insert zero-width space
			}
			result += tag;
		});
...
/**
 * Proxy Telegram media file request.
 * Receive file_id, get temp download link and redirect.
 */
...
/**
 * - Handle Webhook request from Telegram Bot
 * - Video: Save file_id, embed Worker proxy link in content.
 * - Image/File: Re-upload to R2 for permanent availability.
 */
async function handleTelegramWebhook(request, env, secret) {
	...
	try {
		...
		if (!authorizedIdsStr) {
			console.error("Security Warning: AUTHORIZED_TELEGRAM_IDS environment variable is not set.");
			return new Response('OK', { status: 200 });
		}
		...
		// Image processing (keep re-upload)
		if (photo) {
		...
		if (video) {
			if (settings.telegramProxy) {
				...
				// Re-upload mode
				const getFileUrl = `[https://api.telegram.org/bot$](https://api.telegram.org/bot$){botToken}/getFile?file_id=${video.file_id}`;
				const fileInfoRes = await fetch(getFileUrl);
				const fileInfo = await fileInfoRes.json();
				if (!fileInfo.ok) throw new Error(`Telegram getFile API error (video): ${fileInfo.description}`);
				const downloadUrl = `[https://api.telegram.org/file/bot$](https://api.telegram.org/file/bot$){botToken}/${fileInfo.result.file_path}`;
				const fileRes = await fetch(downloadUrl);
		...
		// File processing (mode based on settings)
		if (document) {
			if (settings.telegramProxy) {
				// Proxy mode
				// Note: Cannot display proxy file directly, store meta info only
				filesMeta.push({
				...
				// Could add placeholder, requires frontend rendering
			} else {
				// Re-upload mode
				const getFileUrl = `[https://api.telegram.org/bot$](https://api.telegram.org/bot$){botToken}/getFile?file_id=${document.file_id}`;
				const fileInfoRes = await fetch(getFileUrl);
				const fileInfo = await fileInfoRes.json();
		...
		await updateStmt.bind(
			...
			JSON.stringify(videoObjects), // [New] Bind videoObjects
			noteId
		).run();
...
/**
 * Process note tags, ignoring # in URLs
 */
...
async function handleImgurProxyUpload(request, env) {
	try {
		...
		const imgurResponse = await fetch('[https://api.imgur.com/3/image](https://api.imgur.com/3/image)', {
			method: 'POST',
			headers: {
